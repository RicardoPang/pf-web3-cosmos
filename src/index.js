...
function showWelcome() {
  console.log('======================================');
  console.log('      Cosmos 简易区块链系统');
  console.log('======================================');
  console.log('这是一个基于 Cosmos SDK 的简易区块链系统');
  console.log('支持基本的挖矿、转账和最长链验证等功能');
  console.log('======================================\n');
}
...
async function getNodeConfig() {
  ...
  if (changeConfig.toLowerCase() === 'y') {
    config.httpPort = parseInt(await askQuestion(`HTTP端口 (${config.httpPort}): `)) || config.httpPort;
    config.p2pPort = parseInt(await askQuestion(`P2P端口 (${config.p2pPort}): `)) || config.p2pPort;
